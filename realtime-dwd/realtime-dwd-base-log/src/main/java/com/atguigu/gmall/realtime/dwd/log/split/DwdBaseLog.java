...
public class DwdBaseLog extends BaseApp//继承BaseAPP基类（见common文件夹）
{
    ...
    {
        new DwdBaseLog().start(10011,4,"dwd_base_log", Constant.TOPIC_LOG);//端口号，并行度，消费者主题
    }
    ...
    //TODO ETL数据转换
    private static SingleOutputStreamOperator<JSONObject> etl(DataStreamSource<String> kafkaStrDS)//从jsonStr->jsonObj
    {
        //定义侧输出流标签
        ...
        SideOutputDataStream<String> dirtyDS = jsonObjDS.getSideOutput(dirtyTag);//获取侧输出流中的数据
        //dirtyDS.print("脏数据:");
                ...
                new ProcessFunction<JSONObject, String>()
                {
                    ...
                    public void processElement(JSONObject jsonObj, ProcessFunction<JSONObject, String>.Context ctx, Collector<String> out) throws Exception
                    {
                        ...
                        else
                        {
                            ...
                            Long ts = jsonObj.getLong("ts");//获取时间戳
